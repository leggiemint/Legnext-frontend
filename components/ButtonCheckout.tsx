...
const ButtonCheckout = ({
  priceId,
  mode = "subscription", // Default to subscription for Pro plan
  isFree = false,
}: {
  ...
  const handleAction = async () => {
    ...
    try {
      if (isFree) {
        // For free plan, redirect to authentication/registration
        window.location.href = config.auth.loginUrl;
      } else {
        // For Pro subscription plan, create Stripe checkout session
        if (!priceId) {
