...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = DiffusionSchema.safeParse(body);
    ...
    const { text, callback } = validation.data;

    // Call backend API
    const response = await backendApiClient.createDiffusion(userApiKey.trim(), {
      text,
      ...(callback && { callback }),
    });
    return NextResponse.json(response);
  } catch (error) {
    return createErrorResponse(error, 'Failed to create diffusion');
  }
}
